		...
		<body>
			<div style="width:60%; margin:auto">
				<?php        
					...
					//Shows the cans that you can select
					...
					if(isset($_POST["cid"])) {
						...
						$book_day=mysqli_query($link,"select book_date from room_booking WHERE id_booking = $extract_id");
							while($day = mysqli_fetch_array($book_day)){

								$twomoreweeks = date("Y-m-d", strtotime($day['book_date']. "+14 days"));
								//If you selecta can is going to do an update in the database
								$sql2 = "UPDATE can SET availability = '0', new_use = '$twomoreweeks' WHERE id_can = $cid";
					...
					if(isset($_POST['#canid'])) {
						$canid = $_POST['#canid'];
						//if you diselect a can it will be updatet at the moment and its going to be available again
						$sql2 = "UPDATE can SET availability = '1', new_use = NULL WHERE id_can = $canid";
                ...
                ?>
                    ...
                    <form method="post">
					<!--In each form will appear the id of the booking of this form-->
					...
					<?php
						...
						if(isset($_POST["kage"])) {
					?>
					<p style="text-align:center">Extracted Honey quantity: <input type="number" class="kage" id="kilos" style="width:10vw" max="999" name="kilos" value="<?php echo $_POST["kage"]; ?>"/> KG</p>
					<?php } else { ?>
					<p style="text-align:center">Extracted Honey quantity: <input type="number" class="kage" id="kilos" style="width:10vw" max="999" name="kilos" value="0"/> KG</p>
					<?php } ?>
